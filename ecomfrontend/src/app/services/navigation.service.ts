...
export class NavigationService {
  ...
  deleteCartItem(userId: number, productId: number) {
    const url = `${this.baseurl}DeleteCartItem/${userId}/${productId}`;
  ...
  deletePreviousCart(userid: number, cartid: number){
    let url = this.baseurl + 'DeletePreviousCart/' + userid + '/' + cartid;
    return this.http.delete(url);
  }
  ...
  searchProducts(searchQuery: string): Observable<Product[]> {
    return this.http.get<any[]>(this.baseurl + 'SearchProducts', {
